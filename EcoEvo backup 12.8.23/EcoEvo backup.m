...
EcoEq;SolveEcoEq;NSolveEcoEq;FindEcoEq;TrackEcoEq;FindEcoEqBifurcation;
...
$EcoEvoVersion="1.8.0 (June 10, 2023)";
...
Format[ParametricDynamics[x_,opts___],StandardForm]:=SummaryForm[
	...
	{{"Variables: ",Quiet@Keys[x[[1]]]},{"Domain: ",Quiet@MinMax[ToExpression/@Keys[x]]},{"Type: ",Type/.{opts}},{"ModelType: ",ModelType/.{opts}},{"Length: ",Length[x]}},
	4,StandardForm,x];
...
StyleBox[\"pd\", \"TI\"]\)] returns the smallest parameter value of ParametricDynamics \!\(\*
StyleBox[\"pd\", \"TI\"]\).";


InitialParameter[pd_ParametricDynamics]:=pd["Domain"][[1]];
InitialParameter[pd_?TemporalRuleListQ]:=InitialTime[pd];


FinalParameter::usage="FinalParameter[\!\(\*
StyleBox[\"pd\", \"TI\"]\)] returns the largest parameter value of ParametricDynamics \!\(\*
StyleBox[\"pd\", \"TI\"]\).";


FinalParameter[pd_ParametricDynamics]:=pd["Domain"][[2]];
FinalParameter[pd_?TemporalRuleListQ]:=FinalTime[pd];
...
ExtractParameters[in_(*?RuleListQ*)]:=FilterRules[in,parnames];
...
EcoSimTable[icsin_,tmax_,{par_,parmin_,parmax_,dpar_},opts___?OptionQ]:=

Module[{
(* options *)
verbose,monitor,printtrace,
continuationorder,initialwarmup,warmup,ecosimopts,
(* other variables *)
parname,ics,sol,res
},
...
If[monitor,PrintTemporary[par,"=",Dynamic[par]]];

If[initialwarmup>0,
	ics=Block[{par},par=parmin;EcoSim[icsin,initialwarmup,Output->"FinalSlice",Evaluate[Sequence@@ecosimopts]]],
	ics=icsin
];
...
res=Association@@Table[(ToString@parval)->Block[{par},par=parval;
	If[warmup>0,ics=EcoSim[ics,warmup,Output->"FinalSlice",Evaluate[Sequence@@ecosimopts]]];
	sol=EcoSim[ics,tmax,Evaluate[Sequence@@ecosimopts]];
	Which[
		continuationorder==0,
		ics=FinalSlice[sol]
	];
...
Return[ParametricDynamics[res,Parameter->ToString@par,Type->"Discrete",ModelType->modeltype]]
]];


Options[EcoSimTable]={Verbose->False,Monitor->True,PrintTrace->False,
WarmUp->0,InitialWarmUp->0,ContinuationOrder->0};
...
If[count==maxsteps,Message[TrackEcoEq::mxst,maxsteps,Defer@parname,NumberForm@par]];
...
FindEcoCycle[attributesin:(_?AttributesQ):{},init:(_?VariablesQ):{},Gsin:(_?GsQ):{},opts:OptionsPattern[]]:=
...
FindEcoCycleBifurcation[attributesin:(_?AttributesQ):{},init:(_?VariablesQ):{},Gsin:(_?GsQ):{},par_->parinit_?NumericQ,opts:OptionsPattern[]]:=
...
TrackEcoCycle[init_?VariablesQ,{parname_,parmin_?NumericQ,parmax_?NumericQ,initialstepsize_:Automatic},opts___?OptionQ]:=
...
sol=Check[Quiet[FindEcoCycle[ics,Period->per,Evaluate[Sequence@@findecocycleopts]],FindRoot::lstol],"err"];
(*Print["initial sol=",sol];*)
If[sol==="err",Message[TrackEcoCycle::ferr,Defer@parname,NumberForm@par];Return[$Aborted]];
If[teststability,initialstability=EcoStableQ[sol]];
(*Print["initial ev=",EcoEigenvalues[sol,Multipliers->True]];
...
While[count<maxsteps,
	...
	If[printtrace&&sol=!="err",Print[Defer@parname,"=",NumberForm@par," InitialSlice[sol]=",InitialSlice[sol],
		" Period=",per," ev=",EcoEigenvalues[sol,Multipliers->True]]];
	If[sol=!="err",
		If[modeltype=="ContinuousTime"&&modelperiod==0,
...
(*Print["sol=",sol];*)
	...
	,
		...
		If[dpar>0,par=Min[parmax,parold+dpar],par=Max[parmax,par+dpar]];
	,	
		teststability&&EcoStableQ[sol]=!=initialstability,
		...
		If[Abs[dpar]<Abs[minstepsize],
			If[findbifurcation,
				If[mode=="par",Clear[parname]];
				bif=FindEcoCycleBifurcation[traits,sol,parname->par];
				VPrint[3,"bif=",bif];
				If[mode=="par",
					par=parname=(parname/.bif),
					par=(parname/.bif)
	...
	,
		testbend&&
			Length[tmp]\[GreaterEqual]2&&CompoundOr[Table[
				...
				m0=((unk/.ics\[Prime])-(unk/.tmp\[LeftDoubleBracket]-1,2\[RightDoubleBracket]))/(par-tmp\[LeftDoubleBracket]-1,1\[RightDoubleBracket]);
				list=Table[unk/.ent\[LeftDoubleBracket]2\[RightDoubleBracket],{ent,tmp}];
				180/\[Pi] ArcTan[Abs[(m0-m1)/(1+m0 m1)]]>maxbend
				&&
				Abs[(unk/.ics\[Prime])-(unk/.tmp\[LeftDoubleBracket]-1,2\[RightDoubleBracket])]/(10^-10+Differences[{Min[list],Max[list]}]\[LeftDoubleBracket]1\[RightDoubleBracket])>maxchange
			,{unk,unks}]],
		If[printtrace,Print[Defer@parname,"=",NumberForm@par," Rejected: MaxBend exceeded"]];
		(*If[dpar>0,dpar=Max[minstepsize,dpar/2],dpar=Min[minstepsize,dpar/2]];*)
		dpar=dpar/2;
...
If[count\[Equal]maxsteps,Message[TrackEcoCycle::mxst,maxsteps,Defer@parname,NumberForm@par]];

Clear[parname];

Return[ParametricDynamics[tmp2,Parameter->ToString@parname,Type->"Continuous",ModelType->modeltype]]
]];*)
...
Options[TrackEcoCycle]={
	Verbose->False,Verbosity->1,
	PrintTrace->False,Monitor->True,
	(*InitialStepSize->Automatic,*)InitialSteps->100,
	MinStepSize->Automatic,MaxStepSize->Automatic,MaxSteps->200,MaxRecursion->10,MaxExpansion->3,
...
notOptsEcoJacobian=Except[Alternatives@@Replace[Options[EcoJacobian],h_[a_,_]:>h[a,_],1]];


(* break up combined traitsandpops *)
EcoJacobian[in_List:{},opts___?OptionQ]:=((*Print["breaking"];*)
EcoJacobian[ExtractParameters[in],ExtractAttributes[in],ExtractVarCovs[in],ExtractVariables[in],opts]);


(* make listable across pops *)
EcoJacobian[pars:(_?ParametersQ):{},attributes:(_?AttributesQ):{},Gsin:(_?GsQ):{},variables_?ListOfVariablesQ,opts___?OptionQ]:=(EcoJacobian[attributes,Gsin,#,opts]&/@variables);


EcoJacobian[pars:(_?ParametersQ):{},attributesin:(_?AttributesQ):{},Gsin:(_?GsQ):{},variables:notOptsEcoJacobian:{},opts___?OptionQ]:=
...
"EcoEigenvalues[sol] returns the eigenvalues of the Jacobian matrix of the ecological equations evaluated at sol.
EcoEigenvalues[attributes, sol] uses trait values/interaction coefficients attributes.";*)


(*Options[EcoEigenvalues]={Verbose\[Rule]False,Verbosity->0,
Chop\[Rule]True,Time\[Rule]t,NDSolveOpts\[Rule]{},Multipliers\[Rule]False,Fixed\[Rule]{},IgnoreVar->False};*)


(*notOptsEcoEigenvalues=Except[Alternatives@@Replace[Options[EcoEigenvalues],h_[a_,_]:>h[a,_],1]];*)


(*(* break up combined traitsandpops *)
EcoEigenvalues[in_List:{},opts___?OptionQ]:=((*Print["breaking"];*)
EcoEigenvalues[ExtractParameters[in],ExtractAttributes[in],ExtractVarCovs[in],ExtractVariables[in],opts]);*)


(*(* make listable across pops *)
EcoEigenvalues[pars:(_?ParametersQ):{},attributes:(_?AttributesQ):{},Gsin:(_?GsQ):{},variables_?ListOfVariablesQ,opts___?OptionQ]:=(EcoEigenvalues[attributes,Gsin,#,opts]&/@variables);*)


(*EcoEigenvalues[pars:(_?ParametersQ):{},attributesin:(_?AttributesQ):{},Gsin:(_?GsQ):{},variables:notOptsEcoEigenvalues:{},opts___?OptionQ]:=
...
j=EcoJacobian[pars,attributes,Gsin,variables,Time\[Rule]time,Fixed\[Rule]fixed,IgnoreVar->ignorevar];
VPrint[1,"j=",j];

Which[
	(* DiscreteTime cycle *)
	variables\[NotEqual]{}&&((variables\[LeftDoubleBracket]1,2,0\[RightDoubleBracket]===TemporalData||(modeltype\[Equal]"DiscreteTime"&&modelperiod=!=0))&&time===t),
	res=Eigenvalues[ListMultiplier[Table[j[t\[Prime]],{t\[Prime],j["Times"]}]]]
,
	(* ContinousTime cycle *)
	variables\[NotEqual]{}&&((variables\[LeftDoubleBracket]1,2,0\[RightDoubleBracket]===InterpolatingFunction||(modeltype\[Equal]"ContinousTime"&&modelperiod=!=0))&&time===t),
	dim=Length[j];
	per=FinalTime[variables];
	xsol=NDSolve[{x'[t]\[Equal]j.x[t],x[0]\[Equal]IdentityMatrix[dim]},x,{t,0,per},Sequence@@ndsolveopts]\[LeftDoubleBracket]1\[RightDoubleBracket];
...
(* parse input *)
EcoEigenvalues[in___List:{},opts___?OptionQ]:=Module[{tmp,pars,atts,Gs,vars,varslist},
tmp=Join[in];

pars=ExtractParameters[tmp];
atts=ExtractAttributes[tmp];
Gs=ExtractVarCovs[tmp];
vars=ExtractVariables[tmp];
varslist=Select[Select[tmp,RuleListQ],VariablesQ];
Print["pars=",pars];
Print["atts=",atts];
Print["Gs=",Gs];
Print["vars=",vars];
...
Which[
	method=="RouthHurwitz",
	If[j==="undefined",j=EcoJacobian[attributes,Gsin,variables,Time->time,Fixed->fixed,IgnoreVar->ignorevar]];
...
(*Print["Slice[eq,2.2]=",Slice[eq,2.2]];
Print[\[Lambda][2.2,Slice[eq,2.2]]];

Print["{par,parmin,parmax}=",{par,parmin,parmax}];*)
...
(* no vars given = All *)
PlotEcoEq[eqs_?(*ListOfVariablesQ*)RuleListListQ,{par_,parmin_?NumericQ,parmax_?NumericQ},opts___?OptionQ]:=
Module[{axeslabel,vars},
	axeslabel=Evaluate[AxesLabel/.Flatten[{opts,Options[PlotEcoEq]}]];
	vars=Keys[eqs[[1]]];

(*Print["no vars given = All"];
Print["vars=",vars];*)
