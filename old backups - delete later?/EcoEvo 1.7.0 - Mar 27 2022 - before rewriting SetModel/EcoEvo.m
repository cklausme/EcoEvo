...
$EcoEvoVersion="1.7.0X (January 26, 2022)";
			...
			If[plotvariance==True,
				varplots=DeleteNulls@Table[
					If[Length[LookUp[var]]>1&&LookUp[var][[1]]=="gtrait",
						If[ngcomps[gu]==1,
							varvar=var/.Subscript[trait_, sp_]->Subscript[Var[trait], sp],
							varvar=var/.Subscript[trait_[comp_], sp_]->Subscript[Var[trait][comp], sp]
						];
						(*Print["var=",var];Print["varvar=",varvar];*)
...
(*ExpandGs[rulelist_,nsps_:{}]:=rulelist/.Flatten@Table[
	If[ngcomps[gu]==1,
		If[nsps=={},
			{},
			Table[
				(entry\[Rule]val_)->Sequence@@Table[(Subscript[entry, i]->val),{i,\[FormalCapitalN][gu]/.nsps}]
...
ExpandGs[rulelist_,nsps_:{}]:=rulelist/.Flatten@Table[Table[{
	If[nsps=!={},
		(entry->val_)->Sequence@@Table[(Subscript[entry, i]->val),{i,\[FormalCapitalN][gu]/.nsps}],
		{}
	],
	If[ngcomps[gu]>1,
...
Do[
	...
	Do[
		...
		If[(*transitions===Transitions,*)!transitionbased,
			Print["old style"];
		...
		Print[eqn[Subscript[gcomp, \[FormalI]]]]
	,{gcomp,gcomps[gu]}];
...
If[transitionbased,
	...
	Do[
	If[Global`debug,Print["process=",process]];
		trans=process[[1]];
...
If[Gsin!={},  (* moment-based *)
	If[Global`debug,Print["moment-based"]];
	(* expand Gs *)
	Gs=ExpandGs[Gsin];
	If[Global`debug,Print["Gs=",Gs]];
